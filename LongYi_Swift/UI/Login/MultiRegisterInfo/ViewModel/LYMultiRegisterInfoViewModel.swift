//
//  LYMultiRegisterInfoViewModel.swift
//  LongYi_Swift
//
...
class LYMultiRegisterInfoViewModel: BaseViewModel {
    let clientListRelay = BehaviorRelay<[LYAccountShopModel]>(value: [])
...
extension LYMultiRegisterInfoViewModel {
    ...
    func registerAccount(_ completion: ((LYError) -> Void)?) {
        ...
        API.user.rx.request(User.register(account: account)).subscribe { [weak self] result in
            switch result {
            case .success:
                self?.isAddAccount = true
    ...
    func getClientList(_ completion: ((LYError) -> Void)?) {
        API.user.rx.request(User.clientList(phone: account.phone?.string ?? "")).mapObjectList(LYAccountShopModel.self).subscribe { [weak self] result in
            switch result {
            case .success((let data, _)):
                self?.clientListRelay.accept(data)
                completion?(LYRequestError.success)
            case .error(let error):
                completion?(error as! LYRequestError)
            }
